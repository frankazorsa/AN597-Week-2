...
```
#Homewor1
#peer comment: use ```{r} before your code, and ```after so you can have a shell and see our code results directly.
...
#peer comment: the homework state that the vector is a one word character string. so when you split the sentence, split by space instead of "".
a <- 1:121
...
#peer comment: since you use primates variable and also names(primates) , the first step is useless actually.
